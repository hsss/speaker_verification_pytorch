...
class Network(nn.Module):
	def __init__(self, config):
		...
		#self.negative_mask = torch.ones((config['n_classes'],config['n_classes']), dtype=torch.float32) - nn.init.eye_(torch.empty(config['n_classes'],config['n_classes']))					
	...
	def forward(self, x,  ans, ans2, is_test):
		#x = self.input_drop(x)
		...
		norm = torch.norm(self.fc_output.weight, dim=1, keepdim=True) 
		normed_weight = torch.div(self.fc_output.weight, norm ) 
		
	
		cos_output_tmp = torch.mm(code, normed_weight.t())
		cos_target = cos_output_tmp.gather(1, ans.view(-1,1))
		cos_impo = cos_output_tmp.gather(1, ans2)

		hard_negatives, _ = torch.topk(cos_impo, self.config['negative_k'], dim = 1, sorted = False)
